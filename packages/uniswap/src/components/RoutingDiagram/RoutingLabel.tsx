...
export function RoutingLabel({ trade }: { trade: Trade }): JSX.Element {
  ...
  // SPRY: classic trades on a Spry chain route through the Spry pool via the
  // SpryRouter, not the Uniswap routing API.
  if (!isUniswapX(trade) && !isBridge(trade) && isSpryChain(trade.inputAmount.currency.chainId)) {
    return (
